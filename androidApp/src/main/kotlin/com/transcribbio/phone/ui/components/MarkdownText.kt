...
/** Minimal Markdown renderer: headings, bullets, and **bold** spans. */
@Composable
        ...
        for (raw in lines) {
            ...
            when {
                ...
                line.startsWith("### ") -> Text(inline(line.removePrefix("### ")),
                    style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(top = 6.dp, bottom = 2.dp))
                line.startsWith("## ") -> Text(inline(line.removePrefix("## ")),
                    style = MaterialTheme.typography.titleLarge, color = MaterialTheme.colorScheme.primary,
                    modifier = Modifier.padding(top = 10.dp, bottom = 3.dp))
                line.startsWith("# ") -> Text(inline(line.removePrefix("# ")),
                    style = MaterialTheme.typography.headlineSmall, modifier = Modifier.padding(top = 10.dp, bottom = 4.dp))
                line.startsWith("- ") || line.startsWith("* ") -> BulletLine(line.drop(2), 0)
                line.startsWith("  - ") || line.startsWith("  * ") -> BulletLine(line.drop(4), 1)
                line.startsWith("    - ") || line.startsWith("    * ") -> BulletLine(line.drop(6), 2)
...
private fun BulletLine(text: String, indent: Int) {
    Row(Modifier.padding(start = (8 + indent * 14).dp, top = 1.dp, bottom = 1.dp)) {
...
private fun inline(text: String): AnnotatedString = buildAnnotatedString {
    ...
    while (i < s.length) {
        ...
        if (start == -1) { append(s.substring(i)); break }
        append(s.substring(i, start))
        val end = s.indexOf("**", start + 2)
        if (end == -1) { append(s.substring(start)); break }
